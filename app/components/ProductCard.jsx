...
const ProductCard = ({ product }) => {
    const { name, brand, description, price, discount, status, imageUrl, id } = product;
  ...
  {discount > 0 && (
    <span className='bg-blue-500 absolute z-30 text-grey-light font-bebas text-xl p-2 rounded-full'>
      -{discount}%
    </span>
  )}
...
<div className='p-4 text-gray-600 flex flex-col justify-around flex-grow'>
  ...
  {discount > 0 ? (
    ...
    <div className='flex flex-row items-center space-x-2'>
      <span className='text-primary-red-light text-lg font-bold'>
        ...
        ? 'Disponible' : 'Agotado'
                    ...
                    <div className='flex justify-evenly items-center flex-grow'>
                      ...
                      >Añadir al carrito</Button>
                        ...
                        <Button 
                        className='bg-gray-400 text-grey-light h-10 my-2'
                        variant='disabled'
                      >Agotado</Button>
                        )}
                    <Link href={`/product/${id}`}>
